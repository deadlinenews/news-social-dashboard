...
st.markdown("""
<style>
    .main { background-color: #f8f9fa; color: #2d3748; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    h1, h2, h3 { color: #5a2789 !important; font-weight: 700 !important; }
    .metric-card { background: #ffffff; border-radius: 12px; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.04); border-top: 4px solid #6b46c1; margin-bottom: 15px; }
    .metric-card-orange { border-top: 4px solid #dd6b20; }
    .metric-title { font-size: 13px; text-transform: uppercase; color: #718096; font-weight: 600; margin-bottom: 5px; }
    .metric-value { font-size: 28px; font-weight: 800; color: #1a202c; }
    .stTabs [data-baseweb="tab-list"] { gap: 8px; border-bottom: 2px solid #e2e8f0; }
    .stTabs [data-baseweb="tab"] { height: 45px; background-color: #ffffff; border-radius: 8px 8px 0px 0px; color: #4a5568; font-weight: 600; }
    .stTabs [aria-selected="true"] { background-color: #6b46c1 !important; color: #ffffff !important; }
</style>
...
def clean_data(df):
    ...
    # Standardise platform naming & casing
    platform_map = {
        'twitter': 'X',
        'x': 'X',
        'facebook': 'Facebook',
...
with tab1:
    ...
    with col_chart1:
        ...
        if not filtered_df.empty and "followers" in filtered_df.columns and "platform" in filtered_df.columns:
            # Group by platform to remove duplicates
    ...
    with col_chart2:
        ...
        if not filtered_df.empty and "likes" in filtered_df.columns and "platform" in filtered_df.columns:
            # Group by platform so duplicates like 'twitter' and 'X' combine cleanly
